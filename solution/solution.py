...
def call_openai(
    ...
    latency = max(0.000001, time.time() - start)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        g4 = (r['gpt4o_answer'][:37] + '...') if len(r['gpt4o_answer']) > 40 else r['gpt4o_answer']
        mn = (r['mini_answer'][:37] + '...') if len(r['mini_answer']) > 40 else r['mini_answer']
        g4_t = f"{r['gpt4o_time']:.2f}s"
        mn_t = f"{r['mini_time']:.2f}s"
        lines.append(f"{p:<40} | {g4:<40} | {mn:<40} | {g4_t:<12} | {mn_t:<12}")
    return "\n".join(lines)
